...
public class InputWebsocketTask extends Task<String> {
    private InputWebsocketClient clientEndpoint;

    public InputWebsocketTask(String name) {
        clientEndpoint = new InputWebsocketClient(name);
    }
    ...
    public void connectServer() throws IOException, DeploymentException {
        String SERVER_ENDPOINT_ADDRESS = String.format("ws://localhost:%s/input", DeviceInfo.AGENT_HTTP_SERVER_PORT);
