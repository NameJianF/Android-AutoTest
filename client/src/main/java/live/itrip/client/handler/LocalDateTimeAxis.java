...
public class LocalDateTimeAxis extends Axis<LocalDateTime> {
    ...
    @Override
    public void invalidateRange(final List<LocalDateTime> LIST) {
    ...
    @Override
    protected List<LocalDateTime> calculateTickValues(final double VALUE, final Object RANGE) {
        Object[] range = (Object[]) RANGE;
        LocalDateTime lower = (LocalDateTime) range[0];
        LocalDateTime upper = (LocalDateTime) range[1];
        ...
        if (evenDateList.size() > 2) {
            ...
            if (secondDate.toEpochSecond(ZoneOffset.ofHours(0)) - lower.toEpochSecond(ZoneOffset.ofHours(0)) < thirdDate.toEpochSecond(ZoneOffset
                    .ofHours(0)) - secondDate.toEpochSecond(ZoneOffset.ofHours(0))) {
            ...
            if (upper.toEpochSecond(ZoneOffset.ofHours(0)) - lastDate.toEpochSecond(ZoneOffset.ofHours(0)) < ((lastDate.toEpochSecond(ZoneOffset
                    .ofHours(0)) - previousLastDate.toEpochSecond(ZoneOffset.ofHours(0)) * 0.5))) {
    ...
    private List<LocalDateTime> makeDatesEven(List<LocalDateTime> dates, LocalDateTime calendar) {
        ...
        if (dates.size() > 2) {
            ...
            // This happens if you display days and then add a date many years in the future the tick label will still be
            // displayed as day.
            for (int i = 0; i < dates.size(); i++) {
                calendar = dates.get(i);
                switch (actualInterval.INTERVAL) {
                    case YEARS:
                        // If its not the first or last date (lower and upper bound), make the year begin with first month and
                        // let the months begin with first day.
    ...
    @Override
    public double toNumericValue(final LocalDateTime DATE) {
        return toMillis(DATE);
    }

    @Override
    public LocalDateTime toRealValue(final double VALUE) {
